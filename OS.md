...
두 번째 유형 RR : 응답 시간을 Good -  회전 시간 Bac <br>
...
I/O가 완료되면 Intruppt가 발생하여 CPU가 다른 작업을 수행하게 되고, I/O가 완료되면 Ready Queue에 들어가게 된다.<br>
...
>각 프로세스가 순차적으로 실행되며, 하나의 프로세스가 실행을 완료한 후 다음 프루세스가 실행된다.<br>
>프로세스간 전환 시에는 현재 프로세스 상태를 디스크에 저장하고, 다음 프로세스의 상태를 로드하는 방힉이다.<br>
...
예를 들어, 프로그램의 코드는 주소 공간에 있다. 프로그렘이 실행되는 동안 함수 호출 체인의 위치를 추적하고 지역 변수를 할당하며, 루틴에 매개변수를 전달하고 반환 값을 받기 위해 Stack을 사용한다.<br>
프로그램은 Heap을 사용하여 동적으로 할당된 메모리를 관리한다.<br>

Heap은 메모리의 위쪽에 저장하고 Stack은 메모리의 아래쪽에 저장한다.<br>
이렇데 주소 공잔의 양 끝에 배치함으로써 Heap과 Stack이 서로  커지면서 충돌하는 것을 방지한다.<br>
...
따라서 프로그램은 메모리가 가상화된 사실을 인식하지 않아야하며, 대신 프로그램은 자체 개인적인 물ㄹ적 메모리를 갖고 있는 것처럼 동작해야한다.<br>

2. Viture Memory System의 두 번째 목표는 efficiency(효율성)이다.<br>
...
그런 다음 프로그램이 malloc()을 호출할 때  heap에 정수를 위한 공간을 요청하며, 이 함수느 ㄴ해당 정수의 주소(성공)나 NULL(실패)를 반환한다.<br>

### 14.2 The malloc() Call

heap에 일정한 공간을 요청하는 size를 전달하며느 성공 시 새롭게 할당된 공간을 가리키는 포인터를 반환하고 실패 시 NULL을 반환한다.<br>
...
CPU를 virtualization 가상화하는 과정에서 LDA(limited direct execition)이라고 하는 일반적인 메커니즘에 초점을 맞췄다.<br>
LDA는 대부분의 경우는 프로그램을 하드웨어 위에서 직접 실행하되, 특정한 중요한 시점에 운영 체제가 개입하여 올바른 동작이 이루어지도록 합니다.<br>
...
이 Register 쌍은 주소 공산을 물리 메모리의 원하는 위치에 배치할 수 있게 해주며, 동시에 프로세스가 자신의 즈소 공간에만 접근할 수 있도록 보장한다.<br>
